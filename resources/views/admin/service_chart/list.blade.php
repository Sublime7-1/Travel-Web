 ...
 <div class="search_style">
    ...
    <tbody>
        ...
        <tr>
          ...
          <td>{{$k}}</td>
          <td class="text-l">
          <a href="javascript:;" >{{$value->user_id}}</a>
          <td class="td-status"><span class="label label-success radius">{{$value->service_id}}</span></td>
          <td> <a href="/admin/servicenews/news/{{$value->service_id}}-{{$value->user_id}}" class="btn btn-success btn-xs">进入聊天室</a></td>
          <td> <a onclick="member_del(this,'{{$value->service_id}},{{$value->user_id}}')" class="btn btn-danger btn-xs">删除</a></td>
        </tr>
        @endforeach
        </tbody>
      </table>
...
jQuery(function($) {
    ...
    layer.confirm('确认要删除吗？',function(index){
        $.post("{{ url('admin/servicenews') }}/"+serviceid,{"_token": "{{ csrf_token() }}","_method":"DELETE",'userid':userid},function(data){
