 ...
 <em class="type"></em>
  <div class="shop_logo"><img src="images/detailnoimg.png" /></div>
   <ul class="shop_content clearfix">
    ...
    <li><label class="label_name">所属分类：</label><span class="info">{{$info->type}}</span></li>
    <li><label class="label_name">申请时间：</label><span class="info">{{$info->createtime}}</span></li>
    <li><label class="label_name">状&nbsp;&nbsp;&nbsp;&nbsp;态：</label><span class="info">{{$info->status}}</span></li>
    <li><label class="label_name">申请人姓名：</label><span class="info">{{$info->truename}}</span></li>
    <li><label class="label_name">移动电话：</label><span class="info">{{$info->phone}}</span></li>
    <li><label class="label_name">电子邮箱：</label><span class="info">{{$user->email}}</span></li>
    <li><label class="label_name">固定电话：</label><span class="info">{{$info->g_phone}}</span></li>
...
@if (count($errors) > 0)
        @foreach ($errors->all() as $error)
          <script>layer.msg('{{$error}}',{icon:0,time:1000});</script>
        @endforeach
@endif
<script>
//通过
...
parent.layer.iframeAuto(index);
 function through_save(obj,id){
     layer.confirm('确认要开通该客服吗？',function(index){
        ...
        location.href="/admin/service/success/{{$info->id}}";
    ...
    var index = layer.open({
        ...
        if($('.form-control').val()==""){
              ...
              icon:0,       
                 ...
                 location.href="/admin/service/error/{{$info->id}}";
                 layer.close(index);  
