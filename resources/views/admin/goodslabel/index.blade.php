 ...
 <div id="products_style">
    ...
    <form action="/admin/GoodsLabel" method="get">
      <ul class="search_content clearfix">
       <li><label class="l_f">分类名称</label><input name="keyword" type="text"  value="{{$keyword or ''}}" class="text_add" placeholder="输入分类名称"  style=" width:250px"/></li>
    ...
    @foreach($data as $v)   
     <tr>
        ...
        <td width="80px" label-id="{{$v->id}}">{{$offset+$i++}}</td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick="">{{$v->name}}</u></td>
        <td width="80px">{{$v->type}}</td>
        <td width="80px">{{$v->pid}}</td>
        <td width="250px">{{$v->path}}</td>
        <td class="td-manage">{!!$v->is_display!!}</td>
        <td class="td-manage">
        @if($v->id == 10 || $v->id == 11)
            内置标签分类,不可修改或删除
        @else
        <a title="编辑" onclick="member_open1({{$v->id}})" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      $.post("/admin/GoodsLabel/"+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
            if(data == 1){
