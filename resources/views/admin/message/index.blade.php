     ...
     <div class="search_style"> 
      <form action="/admin/message" method="get">
        ...
        <ul class="search_content clearfix"> 
         <li><label class="l_f">链接名称 :</label><input name="title" type="text" class="text_add" placeholder="  输入链接名称..." value="{{$request->title or ''}}" style=" width:200px;height: 36px" /></li>
      ...
      <table class="table table-striped table-bordered table-hover" id="sample-table"> 
       ...
       @foreach($data as $v) 
        <tr> 
         ...
         <td>{!!$v->content!!}</td> 
         <td>{{$v->u_name}}</td>
         <td class="text-l">{{$v->send_time}}</td> 
         <td class="text-l">{{$v->up_time}}</td> 
         <td class="td-manage"> 
          <a title="编辑" href="javascript:;" onclick="member_edit({{$v->id}})" class="btn btn-xs btn-info"><i class="icon-edit  bigger-120"></i></a>
...
function member_del(obj,id){
  // alert(id);
    ...
    $.post('/admin/message/'+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
      // alert(data);
      if(data == 1){
