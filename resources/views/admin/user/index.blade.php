...
@if (count($errors) > 0)
        ...
        @foreach ($errors->all() as $error)
          <script>layer.msg('{{ $error }}',{icon:6,time:1000})</script>
        @endforeach
...
<div class="page-content clearfix">
    ...
    <tbody>
        ...
        <tr>
          ...
          <td>{{$value->id}}</td>
          <td><u style="cursor:pointer" class="text-primary" onclick="member_show('张三','member-show.html','10001','500','400')">{{$value->username}}</u></td>
          <td>{{$value->sex}}</td>
          <td>{{$value->tel}}</td>
          <td>{{$value->email}}</td>
          <td class="text-l">{{$value->aid}}</td>
          <td>{{$value->time}}</td>
          <td>{{$value->status}}</td>

            @if($value->display == '已开启')
            <td class="td-status"><span class="label label-success radius">{{$value->display}}</span></td>
            @else
          ...
          <td class="td-manage">
            @if($value->display == '已开启')
            <a onClick="member_stop(this,{{$value->id}})"  href="javascript:;" title="停用"  class="btn btn-xs btn-success"><i class="icon-ok bigger-120"></i></a>
            @else
            <a style="text-decoration:none" class="btn btn-xs " onClick="member_start(this,{{$value->id}})" href="javascript:;" title="启用"><i class="icon-ok bigger-120"></i></a>
            @endif
          <a title="编辑"  href="/admin/user/{{$value->id}}/edit"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
          ...
          <td>
            <a href="/admin/user/{{$value->id}}" class="btn btn-success btn-xs">会员详情</a>
            <a href="/admin/address/{{$value->id}}" class="btn btn-info btn-xs">收货地址</a>
          </td>
        </tr>
       @endforeach
      </tbody>
...
$('#tijiao').click(function(){
    ...
    var str="";
     ...
     if($('#password').val() == ''){
      ...
      if(pass.match(/^(?![\d]+$)(?![a-zA-Z]+$)(?![^\da-zA-Z]+$).{8,16}$/) == null){
            layer.alert("密码请输入8-16位的数字和字母！\r\n",{
                    title: '提示框',               
                            icon:0,                             
                }); 
                    num++;
