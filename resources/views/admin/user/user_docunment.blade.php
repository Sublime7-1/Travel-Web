...
<div class="margin clearfix">
 ...
 <div class="search_style">
	...
	<tbody>
        @foreach($userdocument as $v)
		<tr>
        <td><label><input type="checkbox" class="ace"><span class="lbl"></span></label></td>
        <td>{{$v->id}}</td>
        <td>{{$v->uid}}</td>
        <td>{{$v->nickname}}</td>
        <td><a href="javascript:ovid()" onclick="integration_history('23')">{{$v->grow_value}}</a></td>
        <td>{{$status[$v->rank]}}</td>
        
        <td><a href="javascript:ovid()" onclick="Browse_history('13')">{{$v->surf_num}}</a></td>
        <td><a href="javascript:ovid()" onclick="Order_history('33')">{{$v->shop_rate}}</a></td>
