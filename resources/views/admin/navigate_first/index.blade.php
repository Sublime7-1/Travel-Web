...
<div class=" page-content clearfix">
 <div id="products_style">
    <div class="search_style">
     <form action="/admin/NavigateFirst" method="get">
      <ul class="search_content clearfix">
       <li><label class="l_f">分类名称</label><input name="keyword" value="{{$keyword or ''}}" type="text"  class="text_add" placeholder="输入分类名称"  style=" width:250px"/></li>
    ...
    @foreach($data as $v)
     <tr>
        ...
        <td width="80px">{{$offset+$i++}}</td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick="">{{$v->name}}</u></td>
        <td width="100px">第{{$v->level}}层</td>
        <td width="100px">
            @if(is_file('.'.$v->pic))
            <img src="..{{$v->pic}}" width="95px" height="28px" alt="{{$v->name}}"  name="pic" />
            @else
            无
            @endif
        </td>
        <td class="td-manage">
        <a title="编辑" onclick="member_edit('编辑','/admin/navigateFirstAdd/{{$v->id}}','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" >添加子分类</a> 
       </td>
       <td class="td-manage">
        <a title="编辑"  href="/admin/NavigateFirst/{{$v->id}}"  class="btn btn-xs btn-warning" >查看子分类</a> 
         
       </td>
        <td class="td-manage">

        <a title="编辑" onclick="member_edit('编辑','/admin/NavigateFirst/{{$v->id}}/edit','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      $.post("/admin/NavigateFirst/"+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
            if(data == 1){
