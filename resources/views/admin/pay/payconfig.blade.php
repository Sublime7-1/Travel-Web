 ...
 <div class="Configure_style">
    <div class="manner">
      ...
      @foreach($data as $v)
       ...
       <td>
       <label><input name="form-field-radio{{$v->flag}}" onchange="status({{$v->flag}},'0')" type="radio" @if($v->status == 0) checked @endif class="ace" onclick="Enable(this,'123')"><span class="lbl">启用</span></label>
       <label><input name="form-field-radio{{$v->flag}}" onchange="status({{$v->flag}},'1')" type="radio" @if($v->status == 1) checked @endif class="ace" onclick="closes(this,'213')"><span class="lbl">关闭</span></label>     
       </td>      
       <td>{{$v->num}}</td>
        <td><a href="javascript:;" onclick="member_del(this,{{$v->flag}})" class="btn btn-danger" name="" title="支付删除">删除</a></td>
       </tr>
      @endforeach
       </tbody>
      </table>
...
function status(flag,status){
  // alert(id+statuss);
  $.post('/admin/payconfig/'+flag,{'_token':"{{ csrf_token() }}",'_method':'PUT','status':status},function(data){
      // alert(data);
      if(data == 1){
        alert('修改成功');
      }else{
