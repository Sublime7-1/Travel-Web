    ...
    <div class="defray_list cover_style clearfix"> 
     ...
     <div class="defray_content clearfix"> 
      ...
      <ul class="defray_info"> 
       <li class="defray_name">{{$v->name}}</li> 
       <li class="name_logo"><img src="{{$v->img}}" width="100%" height="150px;" /> </li> 
       <li class="description">{{$v->desc}}</li> 
       <li class="select"> 
        <label><input name="form-field-radio{{$v->id}}" type="radio" @if($v->status == 0) checked @endif class="ace" onchange="status({{$v->id}},'0')" /><span class="lbl">启用</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
        <label><input name="form-field-radio{{$v->id}}" @if($v->status == 1) checked @endif onchange="status({{$v->id}},'1')" type="radio" class="ace" /><span class="lbl">关闭</span></label> 
       </li> 

       <li class="operating"> <a href="javascript:ovid()" onclick="member_del(this,{{$v->id}})" class="btn btn-danger"><i class="fa fa-trash"></i>&nbsp;删除</a></li> 
      </ul> 
      @endforeach
     </div> 
    </div> 
...
function status(id,status){
  // alert(id+statuss);
  $.post('/admin/pay/'+id,{'_token':"{{ csrf_token() }}",'_method':'PUT','status':status},function(data){
      // alert(data);
      if(data == 1){
        alert('修改成功');
      }else{
