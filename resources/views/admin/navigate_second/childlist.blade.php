 ...
 <div id="products_style">
    <div class="search_style">
     ...
     <div class="border clearfix">
       <span class="l_f">
        <button onclick="member_edit('编辑','/admin/NavigateSecadd/{{$id}}','4','','510')" title="添加商品" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加分类</button>
        <a href="javascript:void()" class="btn btn-danger"><i class="icon-trash"></i>批量删除</a>
       </span>
       <span class="r_f">共：<b>xxxxxx</b>个子分类</span>
     </div>
    ...
    @foreach($data as $v)
     <tr>
        ...
        <td width="80px" data-id="{{$v->id}}">{{$offset+$i++}}</td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick="">{{$v->name}}</u></td>
        
        <td width="100px">
        @if(is_file('.'.$v->pic))
            <img src="{{$v->pic}}" alt="{{$v->name}}"  width="100px" name="pic" />
        @else
        无
        @endif
        </td>
        <td class="td-manage">{{$v->pid}}</td>
       <td class="td-manage">{{$v->path}}</td>
       <td class="td-manage">{!!$v->is_display!!}</td>
       <td class="td-manage">{{$v->is_popular_game}}</td>
       <td class="td-manage">
         <a title="编辑" onclick="member_edit('添加子分类','/admin/NavigateSecadd/{{$v->id}}','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" >添加子分类</a>
       </td>
       <td class="td-manage">
        <a title="编辑"  href="/admin/NavigateSecond/{{$v->id}}"  class="btn btn-xs btn-info" >查看子分类</a> 
         
       </td>
        <td class="td-manage">
            <a title="编辑" onclick="member_edit('编辑','/admin/NavigateSecond/{{$v->id}}/edit','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      $.post("/admin/NavigateSecond/"+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
            if(data == 1){
