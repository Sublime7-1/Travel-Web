 ...
 <div class="defray_style">
     ...
     <form action="/admin/Goods/" method="get">
	      <ul class="search_content clearfix">
	       <li><label class="l_f">商品名称</label><input name="keyword" type="text"  class="text_add" placeholder="输入商品名称"  value="{{$keyword or ''}}" style=" width:250px"/></li>
      ...
      @foreach($goods as $k=>$v)
       <ul class="defray_info" style="width:240px">
           <li class="defray_name" style="overflow:auto; line-height: 40px; width:100%;font-size:14px">
            <a title="编辑" value="" onclick="member_edit({{$v->id}})" href="javascript:;"  class="" style="float: left; font-size: 25px; color: white; padding-top: 5px; padding-left: 5px;    font-size: 14px;"><i class="fa fa-pencil"></i></a>
            第 {{$k+1}} 件商品
            <button type="button" class="btn btn-info close" style="color:#393939; float: right; line-height: 30px; background:#b0b0b0; width: 22px;height: 22px;align-content: center;line-height: 19px;padding: 1px 1px;" onclick="clock(this,{{$v->id}})"><i class="fa fa-remove" style="font-size: 12px;"></i>
            </button>
            </li>
            <li class="name_logo" style="height:115px"><img src="{{$v->pic}}"  height="100%" width="100%"/> </li>
            <li class="description" style="height: 45px;overflow: hidden;    font-size: 12px;
    padding: 4px 16px;">商品名称：{{$v->title}}</li>
            <li class="select" style="    font-size: 12px;    height: 21px;">
            <label><input name="status{{$v->id}}" type="radio" value="1" onchange="changeStatus(this,{{$v->id}})" class="ace" @if($v->status==1) checked @endif><span class="lbl">上架</span></label>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
             <label><input name="status{{$v->id}}" type="radio" value="2" onchange="changeStatus(this,{{$v->id}})" class="ace" @if($v->status==2) checked @endif><span class="lbl">下架</span></label>
            </li>
            <li class="operating" style="border-bottom: 1px solid #dddddd;">
             <a href="/admin/goodsinfo/travel/{{$v->id}}" class="btn btn-success" style="font-size:12px"><i class="fa  fa-edit "></i>&nbsp;旅游时间</a><a href="/admin/goodsinfo/city/{{$v->id}}" class="btn btn-info" style="font-size:12px"><i class="fa  fa-edit "></i>&nbsp;选择交通</a>
             </li>
...
<div style="margin:0 auto;text-align:center" id="list">{{$goods->appends($request)->render()}}</div>
<!--添加商品-->
...
<div id="add_payment_style" style ="display:none"> 
    ...
    <ul class="margin payment_list  clearfix"> 
      <div class="Operate_cont clearfix">
        ...
        <div class="formControls ">
          <input type="text" class="input-text" id="user-name" name="title" value="{{old('title')}}">
        </div>
      </div>
       <div class="Operate_cont clearfix">
          <label class="form-label" style="width: 107px;"><span class="c-red">*</span>商品价格：</label>
          <div class="formControls">
            <input type="text" class="input-text" placeholder="" value="{{old('price')}}" id="user-name" name="price">
          </div>
       </div>
       <div class="Operate_cont clearfix">
  ...
  <div class="add_menber" id="edit_menber_style" style="display:none"></div>
    @if(session('goods_msg'))
    <script>layer.msg('{{session("goods_msg")}}',{icon: '{{session("tips_code")}}',time:1000});</script>
    @endif
