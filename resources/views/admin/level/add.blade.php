  ...
  @if (count($errors) > 0)
   ...
   <div class="title_name">添加权限</div>
    ...
    <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 用户选择 </label>
       ...
       <label class="middle"><input class="ace top" top="用户选择" checked type="radio" name="userid" value="{{$value->id}}" id="id-disable-check"><span class="lbl"> {{$value->name}}</span></label>
       @endforeach
    </div>   
   </div>
   {{csrf_field()}}
   ...
   <div class="Assign_style">
      ...
      <dl class="permission-list">
        <dt><label class="middle"><input  class="ace top1" type="checkbox" name="ids[]" value="{{$value->ac}}" id="id-disable-check"><span class="lbl">{{$value->typename}}</span></label></dt>
        <dd>

            @foreach($type_two as $val)
            @if($val->pid == $value->id)
             <dl class="cl permission-list2">
             <dt><label class="middle"><input type="checkbox" value="{{$val->ac}}" checked class="ace top1"  name="ids[]" id="id-disable-check"><span class="lbl">{{$val->typename}}</span></label></dt>
                <dd>
                @foreach($type_two as $v)
                @if($v->pid == $val->id)
                
                   <label class="middle"><input type="checkbox" value="{{$val->ac}}{{$v->ac}}" class="ace top1" name="ids[]" id="{{$v->id}}"><span class="lbl">{{$v->typename}}</span></label>
