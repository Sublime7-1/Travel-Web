      ...
      <tr>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{$admin[$k]}}</td>
                <td>1</td>
                <td class="hidden-480">{{$value->userid}}</td>
                <td>{{$value->desc}}</td>
                <td>
                     <a title="编辑" href="/admin/level/{{$value->id}}/edit"  class="btn btn-xs btn-info" ><i class="fa fa-edit bigger-120"></i></a>        
                     <a title="删除" href="javascript:;"  onclick="Competence_del(this,'{{$value->id}}')" class="btn btn-xs btn-warning" ><i class="fa fa-trash  bigger-120"></i></a>
                </td>
            </tr> 
            @endforeach                   
          </tbody>
...
function Competence_del(obj,id){
  layer.confirm('确认要删除吗？',function(index){
        $.post("{{ url('admin/level') }}/"+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
            if(data == 1){
