  ...
  @if (count($errors) > 0)
   ...
   <div class="title_name">添加权限</div>
    ...
    <form action='/admin/level/update' method="post" id="submit2" >
     ...
     <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 权限名称 </label>
       <div class="col-sm-9"><input type="text" id="form-field-1" placeholder="" top="权限名称"  name="username" class="col-xs-10 col-sm-5 top" value="{{$levelinfo->username}}"></div>
    </div>
     <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 权限描述 </label>
      <div class="col-sm-9"><textarea name="desc" class="form-control" id="form_textarea" placeholder="" onkeyup="checkLength(this);">{{$levelinfo->desc}}</textarea><span class="wordage">剩余字数：<span id="sy" style="color:Red;">200</span>字</span></div>
    </div>
    <div class="form-group"><label class="col-sm-2 control-label no-padding-right" for="form-field-1"> 用户选择 </label>
       <div class="col-sm-9">
       @foreach($user as $value)
       <label class="middle"><input class="ace top" top="用户选择" {{$levelinfo->userid == $value->id ? 'checked' : ''}} type="radio" name="userid" value="{{$value->id}}" id="id-disable-check"><span class="lbl"> {{$value->name}}</span></label>
       @endforeach
    </div>   
   </div>
   {{csrf_field()}}
   <input type="hidden" name="_method" value="PUT" />
   <input type="hidden" name="id" value="{{$levelinfo->id}}" />
   ...
   <div class="Assign_style">
      ...
      <dl class="permission-list">
        <dt><label class="middle"><input  class="ace top1" type="checkbox" name="ids[]" {{in_array($value->ac,$ids)  ? "checked" : ""}} value="{{$value->ac}}" id="id-disable-check"><span class="lbl">{{$value->typename}}</span></label></dt>
        <dd>

            @foreach($type_two as $val)
            @if($val->pid == $value->id)
             <dl class="cl permission-list2">
             <dt><label class="middle"><input type="checkbox" value="{{$val->ac}}" {{in_array($val->ac,$ids)  ? "checked" : ""}} class="ace top1"  name="ids[]" id="id-disable-check"><span class="lbl">{{$val->typename}}</span></label></dt>
                <dd>
                @foreach($type_two as $v)
                @if($v->pid == $val->id)
                  
                   <label class="middle"><input type="checkbox" value="{{$val->ac}}{{$v->ac}}" {{in_array($val->ac.$v->ac,$ids)  ? "checked" : ""}} class="ace top1" name="ids[]" id="{{$v->id}}"><span class="lbl">{{$v->typename}}</span></label>       
