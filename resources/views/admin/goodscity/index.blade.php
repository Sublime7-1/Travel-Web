      ...
      <table class="table table-striped table-bordered table-hover" id="sample-table"> 
       ...
       @foreach($goods_city as $k=>$v) 
        <tr> 
         ...
         <td>{{$v->begin_fei}}</td> 
         <td>{{$v->end_fei}}</td> 
         <td>{{$v->begin_fei_time}}</td> 
         <td>{{$v->begin_city}}</td> 
         <td>{{$v->end_city}}</td> 
         <td>{{$v->begin_city_time}} -- {{$v->end_city_time}}</td> 
        
          <td id="shou"><button class="btn btn-success">{{$v->type}}</button></td>
       
         
      
       	 <td>
          <a title="编辑" href="/admin/goodsinfo/city/{{$city_id[$k]['goods_id']}}" class="btn btn-xs btn-info"><i class="icon-edit  bigger-120"></i></a> 
       ...
       <div style="margin:0 auto;text-align:center" id="list">{{ $goods_city->links() }}</div>
      </table> 
    </div> 
    </div> 
   </div> 
