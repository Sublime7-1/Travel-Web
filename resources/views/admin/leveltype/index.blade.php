 ...
 <div id="products_style">
    <div class="search_style">
     ...
     <div class="border clearfix">
       <span class="l_f">
        ...
        <a href="/admin/leveltype/check/{{$pid}}" class="btn btn-warning"><i class="icon-plus"></i>返回上一步</a>
       </span>
    ...
    <tbody>
     ...
     <tr>
        ...
        <td width="80px">{{$value->id}}</td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick="">{{$value->typename}}</u></td>
        <td width="100px">{{$value->pid}}</td>
        <td width="100px">{{$value->path}}</td> 
        <td width="100px">{{$value->ac}}</td> 
        <td class="td-manage">
        <a title="编辑" onclick="member_edit('编辑','/admin/leveltype/mem_add/{{$value->id}}','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" >添加子分类</a> 
       
       </td>
       <td class="td-manage">
        <a title="编辑"  href="/admin/leveltype/check/{{$value->id}}"  class="btn btn-xs btn-warning" >查看子分类</a> 
       
       </td>
        <td class="td-manage">

        <a title="编辑" onclick="member_edit('编辑','/admin/leveltype/{{$value->id}}/edit','4','','510')" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
        <a title="删除" href="javascript:;"  onclick="member_del(this,'{{$value->id}}')" class="btn btn-xs btn-warning" ><i class="icon-trash  bigger-120"></i></a>
       </td>
      </tr>
      @endforeach
    ...
    <div style="margin:0 auto;text-align:center" id="list">{{ $type->links() }}</div>
    </div>     
  </div>
 </div>
</div>
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      ...
      $.post("{{ url('admin/leveltype') }}/"+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
            if(data == 1){
