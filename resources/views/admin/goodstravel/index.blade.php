     ...
     <div class="search_style"> 
      <form action="/admin/coupon/indexs" method="get">
        ...
        <ul class="search_content clearfix"> 
         <li><label class="l_f">商品旅程 :</label><input name="money" type="text" class="text_add" placeholder="  输入数字..." value="{{$request->money or ''}}" style=" width:200px;height: 36px" /></li> 
      ...
      <table class="table table-striped table-bordered table-hover" id="sample-table"> 
       ...
       @foreach($begin_time as $k=>$v) 
        <tr> 
         ...
         <td>{{$v->num}}</td> 
         @if($v->status == '售卖')
          <td id="shou"><button class="btn btn-success" onclick="shou(0)">{{$v->status}}</button></td>
         @else
          <td id="shou"><button class="btn btn-default" onclick="shou(1)">{{$v->status}}</button></td>
         @endif
        
         <td>{!!$content[$k]->content!!}</td>   
       	 <td>
          <a title="编辑" href="/admin/goodsinfo/travel/{{$begin_id[$k]['goods_id']}}" class="btn btn-xs btn-info"><i class="icon-edit  bigger-120"></i></a> 
       ...
       <div style="margin:0 auto;text-align:center" id="list">{{ $begin_time->links() }}</div>
      </table> 
    </div> 
    </div> 
   </div> 
...
function member_del(obj,id){
  // alert(id);
    ...
    $.post('/admin/coupon/'+id,{"_token": "{{ csrf_token() }}","_method":"DELETE"},function(data){
      // alert(data);
      if(data == 1){
