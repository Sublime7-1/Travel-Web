...
<div class="margin clearfix">
 ...
 <div class="article_style">
    <div class="add_content" id="form-article-add">
      <form action="/admin/goodsinfo/travel/{{$goods_id}}" method="post" id="formtip">
        <input type="hidden" name="goods_id" value="{{$goods_id}}" />
        {{csrf_field()}}
        <input type="hidden" name="_method" value="PUT" />
       <ul>
        <li class="clearfix Mandatory" ><label class="label_name" style="width:100px"><i>*</i>商品名称 :</label><span class="formControls col-10"><input style="width:auto" type="text" title="商品名称" value="{{$goods_name}}" id="form-field-1" readonly class="col-xs-10 col-sm-1"></span>
        </li>
       <div id="content">
            ...
            <li class="clearfix cates" ><label class="label_name" style="width:119px"><i>*</i>第{{$num[$k]}}次旅游时间</label>
              <input type="text" autocomplete="off" onclick="status(this)" name="begin_time[]" title="日期" class="laydate-icon start" id="start1{{$k}}" style="width:200px; margin-left:10px; padding-left:5px; margin-right:10px; display:block; float:left" value="{{$v->begin_time}}"></input>
              <em style="display:block; float:left;margin-right:10px;">至</em>
              <input type="text" autocomplete="off" name="end_time[]" onclick="status(this)" class="laydate-icon" id="end11{{$k}}"  value="{{$v->end_time}}" style="width:200px; padding-left:5px; display:block ;float:left" title="日期"></input>
            </li>
            <li class="clearfix Mandatory">
            <label class="label_name"><i>*</i>日程描述 :</label><span class="formControls col-10 end"><input name="content[]" type="text" id="form-field-1" title="日程描述" class="col-xs-10 col-sm-5 " placeholder=" 请输入30字以内的标题..." maxlength="30" value="{{$goods_content[$k]}}"></span>

            </li>
            <li class="clearfix Mandatory">
            <label class="label_name"><i>*</i>发售数量 :</label><span class="formControls col-10 "><input name="num[]" type="text" id="form-field-1" title="数量" class="col-xs-10 col-sm-5 " placeholder="请填写数量" maxlength="30" value="{{$v->num}}"></span>
            </li>
            <li class="clearfix Mandatory">
            ...
            <select name="percent[]" id="form-field-1" title="状态" style="margin-left:10px" class="col-xs-10 col-sm-5 " placeholder="请填写价格" maxlength="30">
                @for($i=10;$i>0;$i--)
                <option {{ $i*10==$v->percent ? 'selected'  : '' }} value="{{$i}}0">{{$i}}0%</option>
                @endfor
            </select>
            </li>
            <li class="clearfix Mandatory">
            ...
            <select name="status[]" id="form-field-1" title="状态" style="margin-left:10px" class="col-xs-10 col-sm-5 " placeholder="请填写数量" maxlength="30" >
                <option  {{ $v->status == 0 ? 'selected'  : '' }} value="0">发售</option>
                <option  {{ $v->status == 1 ? 'selected'  : '' }} value="1">取消发售</option>
            </select>
            </li>
            </div>
            @endforeach
