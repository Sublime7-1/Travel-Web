 ...
 <em class="type"></em>
  <div class="shop_logo"><img src="images/detailnoimg.png" /></div>
   <ul class="shop_content clearfix">
    <li class="shop_name"><label class="label_name">客服名称：</label><span class="info"></span> 
    <div class="Authenticate"><i class="icon-yxrz"></i><i class="icon-yhk"></i><i class="icon-sjrz"></i><i class="icon-grxx"></i></div></li>
    <li><label class="label_name">客服类型：</label><span class="info">普通客服</span></li>
    <li><label class="label_name">所属分类：</label><span class="info"><?php echo e($info->type); ?></span></li>
    <li><label class="label_name">申请时间：</label><span class="info"><?php echo e($info->createtime); ?></span></li>
    <li><label class="label_name">状&nbsp;&nbsp;&nbsp;&nbsp;态：</label><span class="info"><?php echo e($info->status); ?></span></li>
    <li><label class="label_name">申请人姓名：</label><span class="info"><?php echo e($info->truename); ?></span></li>
    <li><label class="label_name">移动电话：</label><span class="info"><?php echo e($info->phone); ?></span></li>
    <li><label class="label_name">电子邮箱：</label><span class="info"><?php echo e($user->email); ?></span></li>
    <li><label class="label_name">固定电话：</label><span class="info"><?php echo e($info->g_phone); ?></span></li>

   </ul>
 </div>
 <div class="Store_Introduction">
  <div class="title_name">客服规定</div>
   <div class="info">
   客服介绍怎么写，只写上一句话或一段话，再加上淘宝平台默认名片式的基本信息，和联系方式。简单明了。例如：
...
淘宝店铺介绍怎么写，你不可能知道每个买家到你的淘宝店铺介绍页面里想了解什么，可以考虑把所有的都写进去。另外，还有购物流程、联系方式、物流方式、售后服务、温馨提示等等都统统写上去。但是一定要花时间好好排版。内容多，字体不能太大，正常就可以了，然后一段内容的标题要加粗或者加上颜色，比如给售后服务加粗，然后售后服务的内容则用正常字体，这样每段内容配上一个加粗标题，买家一点进淘宝店铺介绍，第一眼明显看到的都是几个加粗标题，能很快找到自己想了解的就有耐心看下去。就像本篇文章一样，没有一些加粗的字体，读者不从头读起，就找不到各段内容的主要针对点。
 ...
 <div class="At_button" style="width:272px">
      ...
      <a onclick="return_close();" class="btn btn-default radius" type="button">返回上一步</a>
 </div>
</div>
</body>
</html>
...
parent.layer.iframeAuto(index);
 function through_save(obj,id){
     layer.confirm('确认要开通该客服吗？',function(index){
        
        location.href="/admin/service/success/<?php echo e($info->id); ?>";
...
function return_close(){
    location.href="/admin/servicesend";
    parent.$('#parentIframe').css("display","none");
    parent.$('.Current_page').css({"color":"#333333"});
    ...
    var index = layer.open({
        ...
        if($('.form-control').val()==""){
              ...
              icon:0,       
                 ...
                 layer.msg('已拒绝!',{icon:0,time:1000});
                 location.href="/admin/service/error/<?php echo e($info->id); ?>";
                 layer.close(index);  
