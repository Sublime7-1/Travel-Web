     ...
     <div class="search_style"> 
      <form action="/admin/links" method="get">
        ...
        <ul class="search_content clearfix"> 
         <li><label class="l_f">链接名称 :</label><input name="name" type="text" class="text_add" placeholder="  输入链接名称..." value="<?php echo e(isset($request->name) ? $request->name : ''); ?>" style=" width:200px;height: 36px" /></li> 
      ...
      <table class="table table-striped table-bordered table-hover" id="sample-table"> 
       ...
       <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?> 
        <tr> 
         ...
         <td><img src="<?php echo e($v->brand_img); ?>" title="品牌图" width="100%"></td> 
         <td><img src="<?php echo e($v->show_img); ?>" title="展示图" width="100%"></td> 
         <td class="text-l"><?php echo e($v->company); ?></td> 
         <td><?php echo e($v->contacts); ?></td> 
         <td><?php echo e($v->phone); ?></td> 
         <td><?php echo e($v->email); ?></td> 
         <td><?php echo e($v->area); ?></td> 
         <td><?php echo e($v->apply_time); ?></td> 
         <td class="td-status">
          <?php if($v->status == '未通过'): ?>
          <span class="label radius"><?php echo e($v->status); ?></span></td> 
          <?php else: ?>
          <span class="label label-success radius"><?php echo e($v->status); ?></span></td> 
          <?php endif; ?> 
         <td class="td-manage"> 
          <?php if($v->status == '未通过'): ?>
          <a style="text-decoration:none" class="btn btn-xs btn-success" onclick="member_start(this,<?php echo e($v->id); ?>)" href="javascript:;" title="通过"><i class="icon-ok bigger-120"></i></a>
          <?php else: ?>
          <a style="text-decoration:none" class="btn btn-xs btn-success" onclick="member_stop(this,<?php echo e($v->id); ?>)" href="javascript:;" title="失效"><i class="icon-remove bigger-120"></i></a>
          <?php endif; ?>  
          <a title="删除" href="javascript:;" onclick="member_del(this,<?php echo e($v->id); ?>)" class="btn btn-xs btn-warning"><i class="icon-trash  bigger-120"></i></a> </td> 
        </tr> 
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
       </tbody> 
      </table> 
...
function member_stop(obj,id){
    ...
    $.get('/admin/links/status',{'id':id,'status':0},function(data){
      if(data == 1){
    		$(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" class="btn btn-xs btn-success" onClick="member_start(this,<?php echo e($v->id); ?>)" href="javascript:;" title="通过"><i class="icon-ok bigger-120"></i></a>');
...
function member_start(obj,id){
    ...
    $.get('/admin/links/status',{'id':id,'status':1},function(data){
        if(data == 1){
            $(obj).parents("tr").find(".td-manage").prepend('<a style="text-decoration:none" class="btn btn-xs btn-success" onClick="member_stop(this,<?php echo e($v->id); ?>)" href="javascript:;" title="失效"><i class="icon-remove bigger-120"></i></a>');
...
function member_del(obj,id){
  // alert(id);
    ...
    $.post('/admin/links/'+id,{"_token": "<?php echo e(csrf_token()); ?>","_method":"DELETE"},function(data){
      alert(data);
      if(data == 1){
