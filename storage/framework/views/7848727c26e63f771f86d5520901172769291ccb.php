  ...
  <div class="sort_list">
    <table class="table table-striped table-bordered table-hover" id="sample-table">
		<thead>
		 <tr>
				...
				<th width="70px">添加分类</th>                
				<th width="70px">查看分类</th>                
	...
	<tbody>
      ...
      <tr>
       ...
       <td class="td-manage">
         <a title="添加"  href="javascript:;" value="<?php echo e($v->id); ?>" class="btn btn-xs btn-warning son_add" >添加子分类</a>   
       </td>
       <td class="td-manage">
         <a title="查看" href="/admin/adverttype/check/<?php echo e($v->id); ?>"  class="btn btn-xs btn-info" >查看子分类</a>   
       </td>
       <td class="td-manage td-manages">
        <?php if($v->status == '显示'): ?>
        <a onclick="member_stop(this,<?php echo e($v->id); ?>)"  href="javascript:;" title="隐藏"  class="btn btn-xs btn-success"><i class="fa fa-check  bigger-120"></i></a>
        <?php else: ?>
        <a style="text-decoration:none" class="btn btn-xs " onclick="member_start(this,<?php echo e($v->id); ?>)" href="javascript:;" title="显示"><i class="fa fa-close bigger-120"></i></a>
        <?php endif; ?>   
...
<div class="sort_style_add margin" id="sort_style_add" style="display:none">
	<form action="/admin/adverttype" method="post" id="typeadd" enctype="multipart/form-data">
	<?php echo e(csrf_field()); ?>

  <div class="">
     <ul>
      <li><label class="label_name">分类名称</label><div class="col-sm-9"><input name="name" type="text" id="form-field-1" placeholder="&nbsp;&nbsp;请输入简易的分类名称..." required class="col-xs-10 col-sm-5"></div></li>
      ...
      <li><label class="label_name">放置区域</label>
     	<span class="cont_style"> &nbsp;&nbsp;
     		...
     		<label>
   			...
   			<label>
   				<input name="area" value="5" type="radio" class="ace" /><span class="lbl">友情区域</span>
   			</label>
     	</span>
     </li>

     <li><label class="label_name">分类状态</label>
      <span class="add_content"> &nbsp;&nbsp;<label><input name="status" value="0" type="radio" checked="checked" class="ace"><span class="lbl">显示</span></label>&nbsp;&nbsp;&nbsp;
...
$('.sort_edit').on('click', function(){
	...
	var id=$(this).attr('value');
	// alert(id);
	$.get('/admin/adverttype/'+id+'/edit',{},function(data){
		// console.log(data);
		$('#sort_style_edit').html(data);
		layer.open({
	        type: 1,
...
function member_stop(obj,id){
	...
	layer.confirm('确认要关闭吗？',function(index){
		$.get('/admin/advert/status',{'status':1,'id':id},function(data){
			// alert(data);
            if(data == 1){
                $(obj).parents("tr").find(".td-manages").prepend('<a style="text-decoration:none" class="btn btn-xs " onclick="member_start(this,'+id+')" href="javascript:;" title="显示"><i class="fa fa-close bigger-120"></i></a>');
...
function member_start(obj,id){
	layer.confirm('确认要显示吗？',function(index){
		$.get('/admin/advert/status',{'status':0,'id':id},function(data){
            if(data == 1){
                $(obj).parents("tr").find(".td-manages").prepend('<a style="text-decoration:none" class="btn btn-xs btn-success" onclick="member_stop(this,'+id+')" href="javascript:;" title="隐藏"><i class="fa fa-check  bigger-120"></i></a>');
