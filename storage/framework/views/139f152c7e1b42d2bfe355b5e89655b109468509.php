...
<div class=" page-content clearfix">
 <div id="products_style">
    ...
    <form action="/admin/GoodsLabel" method="get">
      <ul class="search_content clearfix">
       <li><label class="l_f">分类名称</label><input name="keyword" type="text"  value="<?php echo e(isset($keyword) ? $keyword : ''); ?>" class="text_add" placeholder="输入分类名称"  style=" width:250px"/></li>
     ...
     <div class="border clearfix">
       <span class="l_f">
        <a href="/admin/GoodsLabel" class="btn btn-success">查看所有分类</a>
        <button onclick="member_open()" title="添加商品" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加分类</button>
        <a href="javascript:void()" class="btn btn-danger"><i class="icon-trash"></i>批量删除</a>
       </span>
       <span class="r_f">共：<b><?php echo e($num); ?></b>个分类</span>
     </div>
    ...
    <?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>   
     <tr>
        ...
        <td width="80px" label-id="<?php echo e($v->id); ?>"><?php echo e($offset+$i++); ?></td>               
        <td width="250px"><u style="cursor:pointer" class="text-primary" onclick=""><?php echo e($v->name); ?></u></td>
        <td width="80px"><?php echo e($v->type); ?></td>
        <td width="80px"><?php echo e($v->pid); ?></td>
        <td width="250px"><?php echo e($v->path); ?></td>
        <td class="td-manage"><?php echo $v->is_display; ?></td>
        <td class="td-manage">
        <?php if($v->id == 10 || $v->id == 11): ?>
            内置标签分类,不可修改或删除
        <?php else: ?>
        <a title="编辑" onclick="member_open1(<?php echo e($v->id); ?>)" href="javascript:;"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
...
function member_del(obj,id){
    layer.confirm('确认要删除吗？',function(index){
      $.post("/admin/GoodsLabel/"+id,{"_token": "<?php echo e(csrf_token()); ?>","_method":"DELETE"},function(data){
            if(data == 1){
...
function member_open(){
    $.get('/admin/GoodsLabel/create',{},function(data){
        layer.open({
            type:1,
            title:'添加分类',
            area:['700px','400px'],
...
function member_open1(id){
    $.get('/admin/GoodsLabel/'+id+'/edit',{},function(data){
        layer.open({
            type:1,
            title:'修改分类',
            area:['700px','300px'],
...
function changeStatus(obj){
    ...
    var id = $(obj).parents('tr').find('td:nth-child(2)').attr('label-id');
