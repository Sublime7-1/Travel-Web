...
/***/ (function(module, exports, __webpack_require__) {
	...
	var _templateNative = __webpack_require__(1);
	...
	var contentBigData = function () {
	    function contentBigData() {
	        _classCallCheck(this, contentBigData);

	        this.server = {
	            getProductInfo: '//api.tuniu.com/cbd/api/product/getProductInfos'
	        };
	    }

	    _createClass(contentBigData, [{
	        key: 'init',
	        value: function init() {
	            this.initProduct();
	            setTimeout(function () {
	                $('img').each(function () {
	                    var img = $(this);
	                    img.error(function () {
	                        img.attr('src', '//img1.tuniucdn.com/site/images/yj_2016/init-img.jpg');
	                        img.attr('m', '图片加载_失败_帖子详情');
	                    });
	                });
	                $('img').unveil();
	            }, 500);
	        ...
	        key: 'dealWithProductData',
	        value: function dealWithProductData(data) {
	            var str = '';
	            var num = data.tourNum;
	            if (num == 0) {
	                str = "新品上线";
	            } else if (num >= 1 && num <= 9999) {
	                str = num + "人";
	            } else if (num >= 10000 && num <= 999999) {
	                str = (num / 10000).toFixed(1) + '万人';
	            } else if (num > 1000000) {
	                str = parseInt(num / 10000) + '万人';
	            } else {
	                str = "新品上线";
	            }
	            data.tourNum = str;
	            data.satisfactionDegree = Number(data.satisfactionDegree) * 100;
	            data.startPrice = Math.round(data.startPrice);
	            return data;
	        }
	    }, {
	        key: 'dealWithProductName',
	        value: function dealWithProductName(source) {
	            var nameRule = /\<.*?\>/;
	            var tName = '';
	            if (source.indexOf('<') >= 0) {
	                var nbefore = nameRule.exec(source)[0];
	                var nafter = source.replace(nbefore, '');
	                tName = '<span>' + nbefore + '</span>' + nafter;
	            } else {
	                tName = source;
	            }
	            return tName;
	        ...
	        value: function initProduct() {
	            var productIdArr = [];
	            var self = this;
	            $('.J_sdk_product').each(function () {
	                productIdArr.push($(this).data('product-id'));
	            });
	            var pCity = window.cityInfo && window.cityInfo.cityCode || 2500;

	            $.ajax({
	                url: self.server.getProductInfo,
	                data: {
	                    d: JSON.stringify({
	                        bookCityCode: pCity,
	                ...
	                success: function success(res) {
	                    if (res.success) {
	                        res.data.map(function (v) {
	                            v = self.dealWithProductData(v);
	                            v.productName = self.dealWithProductName(v.productName);
	                            var html = (0, _templateNative.template)('sdk-content-trips-template', v);
	                            //用的假数据
	                            $('.J_sdk_product[data-product-id="' + v.productId + '"]').html(html);
	                        });
	                    }
	                }
	            });
	    ...
	    return contentBigData;
	}();

	$(new contentBigData().init());
