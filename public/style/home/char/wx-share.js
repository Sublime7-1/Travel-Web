...
//分享变量
var stageList =["","触发","成功","失败"];
var typeList = ["QQ","新浪微博","微信好友","微信朋友"];
//调用接口收集分享数据
function shareDataCollection(stage,type){
  var shareData = {};
  shareData.stage = stage;
  shareData.shareChannel = type;
  //shareData公共信息添加
  //当前页面链接
  shareData.pageAddress =window.location.href;
  //获取页面title
  try{
      shareData.pageTitle = document.title?document.title:document.getElementsByTagName("title")[0].innerHTML;
      shareData.pageTitle =encodeURIComponent(shareData.pageTitle);
      var xhr = new XMLHttpRequest();
      var shareDataStr = "";
      for(pro in shareData){
        shareDataStr+=pro+"="+shareData[pro]+"&";
      }
      if(shareDataStr.length>0) shareDataStr = shareDataStr.substring(0,shareDataStr.length-1);
      xhr.open('GET',PROTOCOL + "//dynamic.m.tuniu.com/event/share/statistics?"+shareDataStr,true);
...
//APP收集分享数据
function appCallShare(data){
    var dataObj = JSON.parse(data);
    shareDataCollection(dataObj.stage,dataObj.type);
}
...
(function(window, document) {
  try{
      if(navigator.userAgent.toLowerCase().indexOf('micromessenger') > -1){
        ...
        xhr.open('GET',PROTOCOL + "//dynamic.m.tuniu.com/weixin/config/jsApiAjax?url="+url,false);
        xhr.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
  ...
  function getData(){
    return {
        ...
        "imgUrl": document.getElementById('shareImg').getAttribute('src'),
    }
  }

  function formatHtmlTag(str) {
    return str.replace(/\&lt\;/g, '<').replace(/\&gt\;/g, '>');
  }

  function shareWeixin(){
    var shareData = getData();
    //分享给朋友
    wx.onMenuShareAppMessage({
      ...
      trigger: function (res) {
        if(wxShareFn.trigger) wxShareFn.trigger(1);
         shareDataCollection(1,2);
      },
      success: function (res) {
        if(wxShareFn.success) wxShareFn.success(1);
        //为了与event-common兼容
        if(window.wxSuccess) window.wxSuccess(1);
         shareDataCollection(2,2);
      },
      cancel: function (res) {
        if(wxShareFn.cancel) wxShareFn.cancel(1);
         shareDataCollection(3,2);
      },
      fail: function (res) {
       if(wxShareFn.fail) wxShareFn.fail(1);
         shareDataCollection(3,2);
      }
    });
    //分享到朋友圈
    wx.onMenuShareTimeline({
      ...
      imgUrl:shareData.imgUrl,
      trigger: function (res) {
        if(wxShareFn.trigger) wxShareFn.trigger(2);
         shareDataCollection(1,3);
      },
      success: function (res) {
        if(wxShareFn.success) wxShareFn.success(2);
        if(window.wxSuccess) window.wxSuccess(2);
         shareDataCollection(2,3);
      },
      cancel: function (res) {
        if(wxShareFn.cancel) wxShareFn.cancel(2);
         shareDataCollection(3,3);
      },
      fail: function (res) {
       if(wxShareFn.fail) wxShareFn.fail(2);
         shareDataCollection(3,3);
      }
    });
    //分享到QQ
    wx.onMenuShareQQ({
      ...
      imgUrl:shareData.imgUrl,
      trigger: function (res) {
        if(wxShareFn.trigger) wxShareFn.trigger(3);
        shareDataCollection(1,0);
      },
      success: function (res) {
        if(wxShareFn.success) wxShareFn.success(3);
         if(window.wxSuccess) window.wxSuccess(3);
        shareDataCollection(2,0);
      },
      cancel: function (res) {
        if(wxShareFn.cancel) wxShareFn.cancel(3);
        shareDataCollection(3,0);
      },
      fail: function (res) {
       if(wxShareFn.fail) wxShareFn.fail(3);
        shareDataCollection(3,0);
      }
    });
    //分享到微博
    wx.onMenuShareWeibo({
      ...
      imgUrl:shareData.imgUrl,
      trigger: function (res) {
        if(wxShareFn.trigger) wxShareFn.trigger(4);
        shareDataCollection(1,1);

      },
      success: function (res) {
        if(wxShareFn.success) wxShareFn.success(4);
         if(window.wxSuccess) window.wxSuccess(4);
        shareDataCollection(2,1);
      },
      cancel: function (res) {
        if(wxShareFn.cancel) wxShareFn.cancel(4);
        shareDataCollection(3,1);
      },
      fail: function (res) {
       if(wxShareFn.fail) wxShareFn.fail(4);
        shareDataCollection(3,1);
      }
    });
  }
  window.WX = {
    shareConfig:function(data,fn){
      if(data&&data.title) document.getElementById('shareTitle').value = formatHtmlTag(data.title);
      if(data&&data.shareDesc) document.getElementById('shareDesc').value = formatHtmlTag(data.shareDesc);
      if(data&&data.link) document.getElementById('shareLink').value = data.link;
      if(data&&data.imgUrl) document.getElementById('shareImg').setAttribute('src',data.imgUrl);
      if(fn&&fn.trigger) wxShareFn.trigger = fn.trigger;
      if(fn&&fn.complete) wxShareFn.complete = fn.complete;
      if(fn&&fn.success) wxShareFn.success = fn.success;
      if(fn&&fn.cancel) wxShareFn.cancel = fn.cancel;
      if(fn&&fn.fail) wxShareFn.fail = fn.fail;
      shareWeixin();
    }
  };
